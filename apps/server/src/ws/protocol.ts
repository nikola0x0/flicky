...
export type ServerMsg =
  ...
  | {
      ...
      deckHash: string
      /**
       * Number of cards the committed deck actually has — the creator MUST
       * pass this as `deck_size` to `create_duel`, because `reveal_deck`
       * asserts `revealed.length == deck_size` (duel.move). Dynamic under
       * tiered selection (one card per live short/mid market, typically 4);
       * the flat picker uses the fixed round-robin size.
       */
      deckSize: number
    }
  | {
