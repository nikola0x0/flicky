 ...
 * flicky duel referencing it, both players swipe via `record_swipe_deepbook`,
 * and we settle + finalize using DeepBook's `settlement_price()`. Proves the
 * full Move-level integration: our `duel.move` reads `&deepbook_predict::oracle::OracleSVI`
 * spot/forward via the on-chain package, and our `settle_card_deepbook`
 * pulls the canonical settlement price from DeepBook.
 *
 * Notes:
...
async function main() {
  ...
  // Create duel — uses new_card_deepbook for each card.
  let duelId: string
  {
    const tx = new Transaction()
  ...
  console.log("\nswipes (via record_swipe_deepbook):")
  for (let i = 0; i < 5; i++) {
  ...
  console.log("\nsettling cards via settle_card_deepbook…")
  {
    const tx = new Transaction()
    for (let i = 0; i < 5; i++) {
      tx.moveCall({
