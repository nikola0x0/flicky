...
async function simulateRange(oracleId: string, expiryMs: bigint, lower: number, higher: number) {
  const lowerStrike = BigInt(lower) * 1_000_000_000n
  const higherStrike = BigInt(higher) * 1_000_000_000n
  const quantity = 1_000_000n // 1.0 contract
...
async function main() {
  try {
    ...
    const oracleIds = Array.from(new Set(events.data.map(e => (e.parsedJson as any).oracle_id)))
